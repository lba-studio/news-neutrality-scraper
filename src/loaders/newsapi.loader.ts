...
import NewsApiService, {
  NewsApiSource,
} from "../services/news/newsapi.service";
import { NewsService } from "../services/news";
import { Subscriber } from "rxjs";

async function newsApiServiceLoader(subscriber: Subscriber<NewsService>) {
  const countries = ["us", "au"];
  const excludedNewsSources = [
    // this is a cost-saving measure more than anything else, because not a lot of people care about these sources. may need to restrict by category down the line
    "ars-technica",
    "bleacher-report",
    "breitbart-news",
    "crypto-coins-news",
  ...
  sourcesToLookFor.forEach((source) =>
    subscriber.next(NewsApiService.craftNewsServiceFromNewsApiSource(source))
  );
}
