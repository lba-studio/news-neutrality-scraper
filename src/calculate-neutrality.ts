...
async function retrieveScoreFromNewsService(
  ...
  await newsService
    ...
    .pipe(
      map(
        async (news: News): Promise<number | undefined> => {
          return sentimentAnalyzerService
            .analyzeText(news.content)
            .catch((e) => {
              logger.error(
                `Unable to retrieve score for ${newsService.sourceId}:`,
                e
              );
              return undefined;
            });
...
export async function calculateNeutrality() {
  ...
  await loaders
      ...
      map(async (newsService) => {
        ...
        try {
          ...
          if (result === undefined || Number.isNaN(result)) {
            throw new Error(
              `Unable to retrieve score from domain ${newsService.sourceUrl} | ${newsService.sourceId}. Final score: ${result}`
            );
          }
          score = result;
        } catch (e) {
    ...
    .forEach((newsSourceScore) =>
      logger.debug("Calculated and stored news source score:", newsSourceScore)
    );
}
